...
class FeedForwardNeuralNetwork:
    ...
    neural_network = FeedForwardNeuralNetwork(input_dim=10, hidden_dim=32, output_dim=10)
    """
    ...
    def init_params(self):
        ...
        # --> hidden_state = input_vecotr . W_in
        self.W_in = np.random.uniform(-np.sqrt(1. / self.input_dim), np.sqrt(1. / self.input_dim),
                                      (self.input_dim, self.hidden_dim))

        # The output_dim x hidden_dim weight matrix that maps hidden layer to output layer
        # --> output = hidden_state . W_out
        self.W_out = np.random.uniform(-np.sqrt(1. / self.hidden_dim), np.sqrt(1. / self.output_dim),
    ...
    def forward_pass(self, inputs):
        # computing hidden_state: inputs . W_in
        # and applying the tanh activation function.
        hidden_state = np.tanh(np.dot(inputs, self.W_in))
        # computing output: hidden_state . W_out
        # and applying the softmax activation function.
        output = softmax(np.dot(hidden_state, self.W_out))
    ...
    def backpropagate_update(self, input,target_output, learning_rate=0.01):
        ...
        delta2 = delta3.dot(self.W_out.T) * (1 - np.power(hidden_state, 2))
    ...
    def train(self, training_input, training_output, 
        number_of_iterations=100,learning_rate=0.01,batch_size=50):
...
def train_on_bigrams(dr):
    # makes bigram training samples from the data.
    bigram_input, bigram_output = dr.get_trainig_bigrams()

    # Initializing the model
    input_dim, output_dim = len(bigram_input[0]), len(bigram_output[0])
    neural_network = FeedForwardNeuralNetwork(input_dim=input_dim, hidden_dim=256, output_dim=output_dim)


    # training the model
    neural_network.train(bigram_input, bigram_output, number_of_iterations=100)

    # Saving the model
    np.save("index_to_word", dr.index_to_word)
    np.save("W_in", neural_network.W_in)
    np.save("W_out", neural_network.W_out)
...
if __name__ == "__main__":
    ...
    neural_network = FeedForwardNeuralNetwork(input_dim=W_in.shape[0], hidden_dim=W_in.shape[1], output_dim=W_out.shape[1])
